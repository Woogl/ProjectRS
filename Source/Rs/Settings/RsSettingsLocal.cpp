...
#include "RsAudioSettings.h"
//#include "Audio/RsAudioMixEffectsSubsystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(RsSettingsLocal)
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("Rs.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("Rs.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("Rs.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("Rs.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("Rs.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("Rs.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("Rs.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("Rs.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("Rs.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("Rs.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FRsScalabilitySnapshot::FRsScalabilitySnapshot()
{
...
namespace RsSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FRsScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = URsSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : URsSettingsLocal::GetDefaultMobileFrameRate();
	}
...
URsSettingsLocal::URsSettingsLocal()
{
	...
	bEnableScalabilitySettings = URsPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings;

	SetToDefaults();
}

void URsSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
	...
	bEnableLatencyTrackingStats = URsSettingsLocal::DoesPlatformSupportLatencyTrackingStats();
...
void URsSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const URsPlatformSpecificRenderingSettings* PlatformSettings = URsPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ERsFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	RsSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void URsSettingsLocal::ResetToCurrentSettings()
{
...
void URsSettingsLocal::BeginDestroy()
{
...
URsSettingsLocal* URsSettingsLocal::Get()
{
	return GEngine ? CastChecked<URsSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void URsSettingsLocal::ConfirmVideoMode()
{
...
float URsSettingsLocal::GetEffectiveFrameRateLimit()
{
	const URsPlatformSpecificRenderingSettings* PlatformSettings = URsPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == ERsFramePacingMode::DesktopStyle)
	{
...
int32 URsSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return RsSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void URsSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FRsScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void URsSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void URsSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void URsSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void URsSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void URsSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool URsSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
ERsStatDisplayMode URsSettingsLocal::GetPerfStatDisplayState(ERsDisplayablePerformanceStat Stat) const
{
	if (const ERsStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return ERsStatDisplayMode::Hidden;
	}
}

void URsSettingsLocal::SetPerfStatDisplayState(ERsDisplayablePerformanceStat Stat, ERsStatDisplayMode DisplayMode)
{
	if (DisplayMode == ERsStatDisplayMode::Hidden)
	{
...
bool URsSettingsLocal::DoesPlatformSupportLatencyMarkers()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyMarkers);
}

void URsSettingsLocal::SetEnableLatencyFlashIndicators(const bool bNewVal)
{
...
void URsSettingsLocal::SetEnableLatencyTrackingStats(const bool bNewVal)
{
...
void URsSettingsLocal::ApplyLatencyTrackingStatSetting()
{
...
bool URsSettingsLocal::DoesPlatformSupportLatencyTrackingStats()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyStats);
}

float URsSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void URsSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void URsSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float URsSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void URsSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float URsSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void URsSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float URsSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void URsSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float URsSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void URsSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void URsSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 URsSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 URsSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool URsSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 URsSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return RsSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 URsSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return RsSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void URsSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 URsSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const URsPlatformSpecificRenderingSettings* PlatformSettings = URsPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == ERsFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return RsSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void URsSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const URsPlatformSpecificRenderingSettings* PlatformSettings = URsPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ERsFramePacingMode::MobileStyle)
	{
...
void URsSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void URsSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const URsPlatformSpecificRenderingSettings* PlatformSettings = URsPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ERsFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = RsSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void URsSettingsLocal::ClampMobileQuality()
{
	const URsPlatformSpecificRenderingSettings* PlatformSettings = URsPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ERsFramePacingMode::MobileStyle)
	{
...
void URsSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = RsSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void URsSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = RsSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = RsSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString URsSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void URsSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void URsSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool URsSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool URsSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = RsSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool URsSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void URsSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			// TODO:
			// if (URsAudioMixEffectsSubsystem* RsAudioMixEffectsSubsystem = World->GetSubsystem<URsAudioMixEffectsSubsystem>())
			// {
			// 	RsAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			// }
		}
	}
}

bool URsSettingsLocal::CanRunAutoBenchmark() const
{
	const URsPlatformSpecificRenderingSettings* PlatformSettings = URsPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool URsSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void URsSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void URsSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float URsSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void URsSettingsLocal::SetOverallVolume(float InVolume)
{
...
float URsSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void URsSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float URsSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void URsSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float URsSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void URsSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float URsSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void URsSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void URsSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void URsSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void URsSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void URsSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 URsSettingsLocal::GetOverallScalabilityLevel() const
{
...
void URsSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == ERsFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = RsSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void URsSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName URsSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void URsSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const URsAudioSettings* RsAudioSettings = GetDefault<URsAudioSettings>())
			{
				...
				if (UObject* ObjPath = RsAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from Rs Audio Settings."));
					}
				}

				if (UObject* ObjPath = RsAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from Rs Audio Settings."));
					}
				}

				if (UObject* ObjPath = RsAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from Rs Audio Settings."));
					}
				}

				if (UObject* ObjPath = RsAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from Rs Audio Settings."));
					}
				}

				if (UObject* ObjPath = RsAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from Rs Audio Settings."));
					}
				}

				if (UObject* ObjPath = RsAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from Rs Audio Settings."));
					}
				}
			}
		}
...
void URsSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void URsSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const URsPlatformSpecificRenderingSettings* PlatformSettings = URsPlatformSpecificRenderingSettings::Get();
	const TArray<FRsQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FRsQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
...
// 	if (GIsEditor)
// 	{
// 		const URsPlatformEmulationSettings* Settings = GetDefault<URsPlatformEmulationSettings>();
...
// #endif
	...
	{
	case ERsFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case ERsFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case ERsFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void URsSettingsLocal::UpdateConsoleFramePacing()
{
...
void URsSettingsLocal::UpdateDesktopFramePacing()
{
...
void URsSettingsLocal::UpdateMobileFramePacing()
{
...
void URsSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
