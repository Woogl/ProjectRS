...
#include "RsSettingsLocal.generated.h"
...
USTRUCT()
struct FRsScalabilitySnapshot
{
	GENERATED_BODY()

	FRsScalabilitySnapshot();
 ...
 */
UCLASS()
class URsSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	ERsStatDisplayMode GetPerfStatDisplayState(ERsDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(ERsDisplayablePerformanceStat Stat, ERsStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(URsSettingsLocal, FPerfStatSettingsChanged);
	...
	DECLARE_EVENT(URsSettingsLocal, FLatencyFlashInidicatorSettingChanged);
	UFUNCTION()
	...
	DECLARE_EVENT(URsSettingsLocal, FLatencyStatEnabledSettingChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<ERsDisplayablePerformanceStat, ERsStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FRsScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FRsScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(URsSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
