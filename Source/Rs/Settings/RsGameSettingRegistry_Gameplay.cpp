...
UGameSettingCollection* URsGameSettingRegistry::InitializeGameplaySettings(URsLocalPlayer* InLocalPlayer)
{
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			URsSettingValueDiscrete_Language* Setting = NewObject<URsSettingValueDiscrete_Language>();
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<URsSettingsLocal>()->ShouldAutoRecordReplays());

			Setting->AddEditCondition(FWhenPlayingAsPrimaryPlayer::Get());
			//Setting->AddEditCondition(FWhenPlatformHasTrait::KillIfMissing(URsReplaySubsystem::GetPlatformSupportTraitTag(), TEXT("Platform does not support saving replays")));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<URsSettingsLocal>()->GetNumberOfReplaysToKeep());
