...
#include "RsAudioSettings.generated.h"
...
USTRUCT()
struct FRsSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(MinimalAPI, config = Game, defaultconfig, meta = (DisplayName = "RsAudioSettings"))
class URsAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FRsSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FRsSubmixEffectChainMap> LDRAudioSubmixEffectChain;
