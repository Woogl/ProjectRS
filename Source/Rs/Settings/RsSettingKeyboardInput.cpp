...
#include "RsSettingKeyboardInput.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RsSettingKeyboardInput)

class ULocalPlayer;

#define LOCTEXT_NAMESPACE "RsSettings"

namespace Rs::ErrorMessages
{
	static const FText UnknownMappingName = LOCTEXT("RsErrors_UnknownMappingName", "Unknown Mapping");
}


URsSettingKeyboardInput::URsSettingKeyboardInput()
{
	bReportAnalytics = false;
}

FText URsSettingKeyboardInput::GetSettingDisplayName() const
{
	...
	return Rs::ErrorMessages::UnknownMappingName;
}

FText URsSettingKeyboardInput::GetSettingDisplayCategory() const
{
	if (const FKeyMappingRow* Row = FindKeyMappingRow())
	{
		if (Row->HasAnyMappings())
...
const FKeyMappingRow* URsSettingKeyboardInput::FindKeyMappingRow() const
{
...
UEnhancedPlayerMappableKeyProfile* URsSettingKeyboardInput::FindMappableKeyProfile() const
{
...
UEnhancedInputUserSettings* URsSettingKeyboardInput::GetUserSettings() const
{
	if(URsLocalPlayer* RsLocalPlayer = Cast<URsLocalPlayer>(LocalPlayer))
	{
		// Map the key to the player key profile
		if (UEnhancedInputLocalPlayerSubsystem* System = RsLocalPlayer->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
		{
...
void URsSettingKeyboardInput::OnInitialized()
{
...
void URsSettingKeyboardInput::InitializeInputData(const UEnhancedPlayerMappableKeyProfile* KeyProfile, const FKeyMappingRow& MappingData, const FPlayerMappableKeyQueryOptions& InQueryOptions)
{
	check(KeyProfile);
...
FText URsSettingKeyboardInput::GetKeyTextFromSlot(const EPlayerMappableKeySlot InSlot) const
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
...
void URsSettingKeyboardInput::ResetToDefault()
{	
	if (UEnhancedInputUserSettings* Settings = GetUserSettings())
	{
...
void URsSettingKeyboardInput::StoreInitial()
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
		if(const FKeyMappingRow* Row = FindKeyMappingRow())
...
void URsSettingKeyboardInput::RestoreToInitial()
{
...
bool URsSettingKeyboardInput::ChangeBinding(int32 InKeyBindSlot, FKey NewKey)
{
...
void URsSettingKeyboardInput::GetAllMappedActionsFromKey(int32 InKeyBindSlot, FKey Key, TArray<FName>& OutActionNames) const
{	
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{		
...
bool URsSettingKeyboardInput::IsMappingCustomized() const
{
	bool bResult = false;
