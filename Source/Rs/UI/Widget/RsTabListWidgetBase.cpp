...
bool URsTabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FRsTabDescriptor& OutTabInfo)
{
	const FRsTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FRsTabDescriptor& TabInfo) -> bool
	{
...
void URsTabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FRsTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool URsTabListWidgetBase::RegisterDynamicTab(const FRsTabDescriptor& TabDescriptor)
{
...
bool URsTabListWidgetBase::IsFirstTabActive() const
{
...
bool URsTabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool URsTabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 URsTabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void URsTabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void URsTabListWidgetBase::NativeConstruct()
{
...
void URsTabListWidgetBase::NativeDestruct()
{
	for (FRsTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void URsTabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FRsTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void URsTabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void URsTabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FRsTabDescriptor* TabInfoPtr = nullptr;
	
	FRsTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(URsTabButtonInterface::StaticClass()))
	{
		...
		{
			IRsTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
void URsTabListWidgetBase::SetupTabs()
{
	for (FRsTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
