...
void URsConfirmScreen::SetupDialog(UCommonGameDialogDescriptor* Descriptor, FCommonMessagingResultDelegate ResultCallback)
{
	...
	EntryBox_Buttons->Reset<URsButtonBase>([](URsButtonBase& Button)
	{
	...
	for (const FConfirmationDialogAction& Action : Descriptor->ButtonActions)
	{
		...
		URsButtonBase* Button = EntryBox_Buttons->CreateEntry<URsButtonBase>();
...
void URsConfirmScreen::KillDialog()
{
	Super::KillDialog();
}

void URsConfirmScreen::NativeOnInitialized()
{
...
void URsConfirmScreen::CloseConfirmationWindow(ECommonMessagingResult Result)
{
	DeactivateWidget();
	OnResultCallback.ExecuteIfBound(Result);
}

FEventReply URsConfirmScreen::HandleTapToCloseZoneMouseButtonDown(FGeometry MyGeometry, const FPointerEvent& MouseEvent)
{
	FEventReply Reply;
...
#if WITH_EDITOR
void URsConfirmScreen::ValidateCompiledDefaults(IWidgetCompilerLog& CompileLog) const
{
