...
#include "RsTabListWidgetBase.generated.h"
...
class URsTabButtonInterface : public UInterface
{
	GENERATED_BODY()
};

class IRsTabButtonInterface
{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintNativeEvent, Category = "Tab Button")
	void SetTabLabelInfo(const FRsTabDescriptor& TabDescriptor);
};

UCLASS(MinimalAPI, Blueprintable, BlueprintType, Abstract, meta = (DisableNativeTick))
class URsTabListWidgetBase : public UCommonTabListWidgetBase
{
	GENERATED_BODY()
	
public:
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Tab List")
	UE_API bool GetPreregisteredTabInfo(const FName TabNameId, FRsTabDescriptor& OutTabInfo);

	/** Helper method to get at all the preregistered tab infos */
	const TArray<FRsTabDescriptor>& GetAllPreregisteredTabInfos() { return PreregisteredTabInfoArray; }
	...
	UFUNCTION(BlueprintCallable, Category = "Tab List")
	UE_API bool RegisterDynamicTab(const FRsTabDescriptor& TabDescriptor);
	...
	DECLARE_EVENT_TwoParams(URsTabListWidgetBase, FOnTabContentCreatedNative, FName /* TabId */, UCommonUserWidget* /* TabWidget */);
...
private:
	...
	TArray<FRsTabDescriptor> PreregisteredTabInfoArray;
	 ...
	 */
	UPROPERTY()
	TMap<FName, FRsTabDescriptor> PendingTabLabelInfoMap;
};
