...
bool URsMVVMGameSubsystem::AddSingletonViewModel(UMVVMViewModelBase* ViewModel, bool bWarnIfNotFound)
{
	...
	{
		UE_LOG(LogRsUI, Warning, TEXT("Failed to AddSingletonViewModel(): %s"), *ViewModelClassName.ToString());
	}
	return bSuccess;
}
...
UMVVMViewModelBase* URsMVVMGameSubsystem::GetSingletonViewModel(TSubclassOf<URsViewModelBase> ViewModelClass, bool bWarnIfNotFound)
{
	...
	if (bWarnIfNotFound)
	{
		UE_LOG(LogRsUI, Warning, TEXT("Failed to GetSingletonViewModel(): %s"), *GetNameSafe(ViewModelClass));
	}
	return nullptr;
}
